...
for col in COdf[
    ['MONOXIDE', 'CONTROL', 'HEATTYPE', 'HEATFUEL', 'HOTWATER', 'ACPRIMARY', 'ACSECNDRY', 'HOA', 'RENTCNTRL', 'HHMAR',
     'HHGRAD', 'HHRACE', 'INTLANG']]:
...
df = COdf.fillna(-10)
df = df[(df['MONOXIDE'] >= 0) & (df['HOA'] >= 0) & (df['YRBUILT'] >= 0) & (df['RENT'] >= 0) & (df['RENTCNTRL'] >= 0) & (
            df['HHMAR'] >= 0) & (df['HHGRAD'] >= 0) & (df['HHRACE'] >= 0) & (df['INTLANG'] >= 0) & (
                    df['WEIGHT'] >= 0) & (df['HEATTYPE'] >= 0) & (df['HEATFUEL'] >= 0) & (df['HOTWATER'] >= 0) & (
                    df['ACPRIMARY'] >= 0) & (df['ACPRIMARY'] >= 0) & (df['ACSECNDRY'] >= 0) & (df['HINCP'] >= 0)]
...
features = ['MONOXIDE', 'CONTROL', 'HEATTYPE', 'HEATFUEL', 'HOTWATER', 'ACPRIMARY', 'ACSECNDRY', 'HOA', 'RENTCNTRL',
            'HHMAR', 'HHGRAD', 'HHRACE', 'INTLANG']
for i in features:
    col_histogram(df, i)
detectorscount = COdf[(COdf['MONOXIDE'] >= 0)]
detectorscount = detectorscount.groupby('MONOXIDE').MONOXIDE.count()
# Plot of CO detector status
...
plt.show()
# The initial model will look at household income alone as a predictor of CO monitor status
...
# Now the model will look at all of the features
df_model1 = COdf[(COdf['MONOXIDE'] >= 0) & (COdf['HOA'] >= 0) & (COdf['YRBUILT'] >= 0) & (COdf['RENT'] >= 0) & (
            COdf['RENTCNTRL'] >= 0) & (COdf['HHMAR'] >= 0) & (COdf['HHGRAD'] >= 0) & (COdf['HHRACE'] >= 0) & (
                             COdf['INTLANG'] >= 0) & (COdf['WEIGHT'] >= 0) & (COdf['HEATTYPE'] >= 0) & (
                             COdf['HEATFUEL'] >= 0) & (COdf['HOTWATER'] >= 0) & (COdf['ACPRIMARY'] >= 0) & (
                             COdf['ACPRIMARY'] >= 0) & (COdf['ACSECNDRY'] >= 0) & (COdf['HINCP'] >= 0)]
...
# Cross validation pipeline function to increase robustness of models.
...
pipe_rf = Pipeline([('StandardScaler', ss), (
'Random Forest', RandomForestClassifier(n_estimators=100, max_depth=10, min_samples_leaf=10, random_state=0))])
...
Model1_dict = dict(
    zip(df_model1.drop(columns=[model1target]).columns, pipe_rf.steps[1][1].fit(X1, y1).feature_importances_))
SortModel1 = pd.Series(Model1_dict).sort_values(ascending=True)
print(SortModel1)
SortModel1 = SortModel1.reset_index()
print(
    'According to the random forest model, the most important features are income, rent status, education level, heat type, and year built')
plt.xlabel("Relative Importance")
plt.ylabel('Features')
plt.yticks(range(len(list(SortModel1['index']))), list(SortModel1['index']))
ax.invert_yaxis()
importances = pd.Series(
    RandomForestClassifier(n_estimators=100, max_depth=10, min_samples_leaf=10, random_state=0).fit(X1,
                                                                                                    y1).feature_importances_)
...
Model1v2_dict = dict(
    zip(df_model1.drop(columns=[model1target]).columns, (pipe_logit.steps[1][1].fit(X1, y1).coef_).flat))
SortModel1v2 = pd.Series(Model1v2_dict).sort_values(ascending=True)
print(SortModel1v2)
print(
    "As seen above, higher income, gas-fueled appliances, lower rent, older homes, and lower education levels increase the likelihood of a household with a CO monitor")
# Now lets see if ensembling improves the accuracy
from sklearn.ensemble import VotingClassifier

pipe_ensemble = Pipeline([('StandardScaler', ss), (
'Vote', VotingClassifier(estimators=[('Logit', pipe_logit), ('rf', pipe_rf), ('svc', pipe_svm)], voting='hard'))])
...
print('Since the ensembling method did not improve accuracy, we will analyze the dataset using the random forest model')
